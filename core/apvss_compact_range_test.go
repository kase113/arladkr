...
func TestAPVSSCompactInnerProductDeterministicVectorV1(t *testing.T) {
	generators, err := apvssCompactRangeGeneratorsForV1(4)
	if err != nil {
		t.Fatal(err)
	}
	...
	prefix := hashBytes([]byte("ARL-APVSS-v1/compact-range/cross-vector"), []byte{0, 1, 2, 3})
	inner := apvssCompactInnerProductV1(a, b)
	p := apvssCompactPointSumV1(generators.g, a)
	p.Add(&p, pointPtr(apvssCompactPointSumV1(generators.h, b)))
	p.Add(&p, pointPtr(cvPointTimes(&generators.u, &inner)))
	proof, err := apvssProveCompactInnerProductV1(
		prefix, generators.g, generators.h, generators.u, a, b,
	)
	if err != nil {
		t.Fatal(err)
	}
	if err := apvssVerifyCompactInnerProductV1(
	...
	got := hex.EncodeToString(hashBytes(
		[]byte("ARL-APVSS-v1/compact-range/cross-vector/digest"), wire.Bytes(),
	))
...
func apvssCompactRangeFixtureV1(t testing.TB, values []uint64, bits int) ([]byte, []fr.Element, []bls12381.G1Affine) {
	t.Helper()
	...
	for i := range values {
		var err error
		blindings[i], err = apvssRandomFrV1()
		if err != nil {
			t.Fatal(err)
		}
		commitments[i], err = apvssCompactRangeCommitmentV1(values[i], blindings[i])
		if err != nil {
			t.Fatal(err)
		}
	}
...
func apvssCloneCompactRangeProofV1ForTest(in *apvssCompactRangeProofV1) *apvssCompactRangeProofV1 {
	if in == nil {
		return nil
	}
	out := *in
...
func TestAPVSSCompactRangeProofV1(t *testing.T) {
	...
	statement, blindings, commitments := apvssCompactRangeFixtureV1(t, values, 8)
	proof, err := apvssProveCompactRangeV1(statement, commitments, values, blindings, 8)
	if err != nil {
		t.Fatal(err)
	}
	if err := apvssVerifyCompactRangeV1(statement, commitments, proof, 8); err != nil {
		t.Fatalf("valid aggregated range proof rejected: %v", err)
	}
	wire, err := apvssCompactRangeProofV1CanonicalBytes(proof)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAPVSSCompactRangeProofRejectsMutationV1(t *testing.T) {
	...
	t.Run("polynomial response", func(t *testing.T) {
		bad := apvssCloneCompactRangeProofV1ForTest(proof)
		bad.tHat.Add(&bad.tHat, &one)
		if err := apvssVerifyCompactRangeV1(statement, commitments, bad, 8); err == nil {
	...
	t.Run("statement", func(t *testing.T) {
		...
		badStatement[0] ^= 1
		if err := apvssVerifyCompactRangeV1(badStatement, commitments, proof, 8); err == nil {
	...
	t.Run("commitment", func(t *testing.T) {
		...
		if err := apvssVerifyCompactRangeV1(statement, badCommitments, proof, 8); err == nil {
...
func TestAPVSSCompactRangeProofRejectsOutOfRangeWitnessV1(t *testing.T) {
	values := []uint64{256}
	statement, blindings, commitments := apvssCompactRangeFixtureV1(t, values, 8)
	if _, err := apvssProveCompactRangeV1(statement, commitments, values, blindings, 8); err == nil {
