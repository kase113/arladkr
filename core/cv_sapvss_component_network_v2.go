...
func (s *cvAPDBNetworkServiceV2) VerifiedComponentLeavesV2(
	...
	payloads := make([][]byte, len(selected))
	leafDigests := make([][]byte, len(selected))
	...
	for i, poolIndex := range selected {
		...
		payloads[i] = append([]byte(nil), entry.payload...)
		leafDigests[i] = append([]byte(nil), entry.leafDigest...)
	}
	s.mu.Unlock()
	leaves := make([]*cvLeafV2, len(payloads))
	for i, payload := range payloads {
		leaf, err := cvDecodeLeafV2(payload, s.cfg.LeafContext, s.cfg.Receivers, s.cfg.Validators)
		if err != nil || !bytes.Equal(leaf.Digest, leafDigests[i]) {
			if err == nil {
				err = fmt.Errorf("CV V2 verified component leaf digest changed")
			}
			return nil, err
		}
		leaves[i] = leaf
